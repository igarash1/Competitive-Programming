...
#include <algorithm>
using namespace std;
inline void getmin(int &x,int y) { if(y<x) x = y; }
#define debug() printf("%d\n",__LINE__)

struct p { int x, y, ix; };

int lxly(const p& a,const p &b) {
  if(a.x == b.x && a.y == b.y) return a.ix<b.ix;
...
int main() {
  ...
  for(i=0;i<n;i++) pps[ps1[i].ix] = bsearch(ps1[i].y)+1;
  maxy++;

  sort(ps1,ps1+n,lxly);
  sort(ps2,ps2+n,lxgy);

  for(i=0;i<=maxy;i++) A[i]=B[i]=C[i]=D[i]=-100000000;

  for(i=0,j=n-1;i<n;i++,j--) {
    getmin(ans[ps1[i].ix], 
	   ps1[i].x + ps1[i].y-query(pps[ps1[i].ix]+1, A) );
    ...
    getmin(ans[ps2[j].ix],
	   -ps2[j].x +ps2[j].y-query(pps[ps2[j].ix]+1, C));
